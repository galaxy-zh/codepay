...
public function cash()
		...
		if ($_POST) {
			...
			if (empty($invest)) {
				...
				$msg = (explode(",",$v['allowable']));
					...
					$err_msg = '请在 '.$msg['0'].':00-'.$msg['1'].':00 提交申请!';
					$week = date('w', $time);
					$hour = date('H', $time);
			...
			$data = array('uid' => $uid, 'name' => $user['name'], 'bid' => $bid, 'bank' => $bank['bank'], 'account' => $bank['account'], 'money' => $money, 'status' => 0, 'time' => date('Y-m-d H:i:s'), 'time2' => '0000-00-00 00:00:00');
					...
					if($userCount >= $v['withdrawals']){
						if (addData('cash', $data)) {
							$Charge = ($money * $v['charged']) + $money;//扣除的总金额
							$Charge = round($Charge,2); //总金额保留两位小数
							$shouxufei = ($money * $v['charged']);//算出手续费多少钱
							$shouxufei = round($shouxufei,2); //总金额保留两位小数
						
							addFinance($uid, $Charge, '余额提现' . $money . '元手续费:'.$shouxufei.'', 2, getUserField($uid, 'money'));
	...
	public function tuiguang()
	{
		...
		if($user){
			...
			//	var_dump($value['id']);
				$arr[] = getData('user', 'all', 'top = \'' . $value['id'] . '\'');
				//$ard = getData('user', 'all', 'top = \'' . $value['id'] . '\'');
				
			}

			if(($arr)){
				foreach($arr as $key=>$value){
							if(count($value) != 0){ //判断 如果没下限的不循环
								for ($x=0; $x<=count($value); $x++) {
										if($value[$x]['id'] != ''){
											$arr[]= getData('user', 'all', 'top = \'' . $value[$x]['id'] . '\'');
		...
		$str = '';
			foreach($arr as $key=>$value){ //取出所有下限id
				if(count($value) != 0){ //判断 如果没下限的不循环
					for ($x=0; $x<=count($value); $x++) {
						if($value[$x]['id'] != ''){
					$str .= ($value[$x]['id']).',';
    ...
    $data = trim($data,',');//trim — 去除字符串首尾处的空白字符（或者其他字符）,假如不使用，后面会多个逗号  
		...
		$finance = M("recharge"); // 实例化User对象
		$user = M("user"); // 实例化User对象
		$cash = M("cash"); // 实例化User对象
		$where['id'] = array("in",$data);
		$wherea['status'] = '1';
		
		$user_res = $user->where($where)->order('time desc')->select(); //查询user表把所有下限统计出来 然后再计算
	//	$results = $finance->where($where)->order('time desc')->select();
		//$cash = $cash->where($where)->order('time desc')->select('money');
		
		foreach($user_res as $key=>$value){
			
			$wherea['uid'] =$value['id'];
			$user_res[$key]['sum'] = $finance->where($wherea)->sum('money');
			$user_res[$key]['tixiansum'] = $cash->where($wherea)->sum('money');
				 
		}
			//var_dump($user_res);
	   
	
		$this->assign('invest', $user_res);
	...
	public function addzhuanru(){
		...
		$res = getData('user', 1, 'id = \'' . $uid . '\'');
			...
			if ($result) {
					$data = array('uid' => $uid, 'paypal' => $money, 'gupaypal' => $money, 'time' => time(),'dayyuebao'=>'0');
							if(!getData('yuebao', 1, 'uid = \'' . $uid . '\'')){
								if (addData('yuebao', $data)) {
									addFinance($uid, $money, '转入余额宝' . $money . '元', 2, $res['money']);
								...
								if ($yuebao->where(['uid'=>$uid])->setInc('paypal',$money)) {
										...
										$User = M(); // 实例化User对象
										$sqla="update yuebao set gupaypal=gupaypal+{$money} WHERE uid={$uid}";
										$resa = $User->execute($sqla);
	...
	public function addzhuanchu(){
		...
		if($result){
			if($User->where(['id'=>$uid])->setInc('money',$money)){
				addFinance($uid, $money, '转出余额宝' . $money . '元', 1, $res['money']);
