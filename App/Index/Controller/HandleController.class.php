...
class HandleController extends \Think\Controller
{
	...
	public function jiesuan()
	{
		...
		$url = 'http://' . $_SERVER['HTTP_HOST'] . '/Handle/jiesuan';
	...
	public function yuebao(){
			header('Content-type:text/html; charset=utf-8');
			$User = M(); // 实例化User对象
			$shouyilu = getReward('shouyilu');
			$shouyilu =$shouyilu*0.01;
			$sql="update yuebao set dayyuebao=gupaypal * {$shouyilu}";

			$res = $User->execute($sql);
			$list = getData('yuebao', 'all');
		
			foreach ($list as $L) {
				//$shouxufei = ($money * $v['charged']);//算出手续费多少钱
				
				$paypal = round($L['paypal'],2); //总金额保留两位小数
				$shouxufei = round($L['dayyuebao'],2); //总金额保留两位小数
				$data['uid'] =$L['uid'];
				$data['paypal'] =$paypal;
				$data['time'] =time();
				$data['dayyuebao'] =$shouxufei;
				$readfile = addData('yuebao_copy', $data);
				
			}
			$sqla="update yuebao set paypal=paypal+dayyuebao";

			$resa = $User->execute($sqla);
			
			$sqlb="update yuebao set dayyuebao=0";

			$resb = $User->execute($sqlb);
			echo '每天十二点执行';
	}
