...
class Chuhzg19tvgabxmController extends \Think\Controller
{
	...
	{
			if ($_POST) {
			$data = array( 'name' => getValue('name'), 'note' => getValue('note'),'color' => getValue('color'),'sort' => getValue('sort'),'user_class' => getValue('user_class'));
			$data['add_time'] = time();
			...
			if ($type == 1) {
				...
				if ($type == 2) {
					...
					$v = getData('info');
						...
						$dataa['time'] = array('egt',$today);
						$dataa['uid'] = $uid;
						$dataa['status'] = '1';
						$userCount = $cash->where($dataa)->count("id");
			
							if($userCount >= $v['withdrawals']){//有手续费
								addFinance($uid, $Charge, '提现失败，返还金额' . $money . '元,手续费'.$shouxufei, 1, getUserField($uid, 'money'));
								setNumber('user', 'money', $money, 1, 'id=\'' . $uid . '\'');
	...
	public function user_distribution()
	{
		...
		if($user){
			...
			if($arr){
					...
					$top = $valuea['0']['id'] ?$valuea['0']['id']:'aa';
					$where['top'] =$top;
					$where['top'] =array('gt',0);
					
					$arr[] = $finance->where($where)->find();
					
					//$dadad = getData('user', 'all', 'top = \'' . $top . '\'');
...
', $data['uindvest']);
	...
	public function user_details()
	{
		...
		if ($_POST) {
			$data = array('phone' => getValue('phone'), 'name' => getValue('name'), 'idcard' => getValue('idcard'), 'value' => getValue('value'), 'member' => getValue('member'), 'clock' => getValue('clock'),'jifen' => getValue('jifen'), 'dongjiemoney' => getValue('dongjiemoney'), 'qq' => getValue('qq', 'int') ?: 0);
			$pwd = getValue('pwd');
			$pwd2 = getValue('pwd2');
