...
public function project()
	...
	public function details()
	{
		...
		$now = date('Y-m-d H:i:s');
			...
			$class_res = getData('project_class', 1, 'id=\'' . $data['class'] . '\'');
		
			$uid = $_SESSION['uid'];	
				$user_res = getData('user', 1, 'id=\'' . $uid . '\'');
			
				$class_res = getData('project_class', 1, 'id=\'' . $class_res['id'] . '\'');
				if($user_res['member'] < $class_res['user_class']){
					msg('您的等级不够', 2, U('login'));
	...
	public function zhuce()
	{
		if ($_POST) {
			...
			$reward = getData('reward', 1);
			$data = array('phone' => $phone, 'password' => md5($pwd), 'password2' => md5($pwd), 'top' => $tid, 'member' => 8001, 'logintime' => time(), 'money' => $reward['register'] ?: 0, 'clock' => 0, 'value' => 0, 'time' => date('Y-m-d H:i:s'));
			$uid = addData('user', $data);
		...
		public function yuebao()
	{
		...
		$m=M('yuebao_copy');
        $c=$m->where(['uid'=>$uid])->sum('dayyuebao');
		$this->assign('res', $res);
		$this->assign('c', $c);
		$this->display();
	}
